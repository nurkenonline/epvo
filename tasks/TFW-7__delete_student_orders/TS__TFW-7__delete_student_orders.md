...
- Практические наблюдения из ИС ОВПО (пример реализации)
...
На основе практической реализации ИС ОВПО описать:
...
- **ID-формула с month_offset:** в ИС ОВПО используется формула `base + studentId + (month-1)*1M` для уникальности ID при множественных месяцах
