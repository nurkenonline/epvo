...
> ❓ TRANSCRIPT при type=13: зависит от реализации ИС ОВПО. В AD ЕПВО `orders_importer.py` создаёт TRANSCRIPT с `not_included_scholarship=True`, но `scholarship_csv.py` — нет. Удалять только если TRANSCRIPT был создан.
...
- AD ЕПВО: `scholarship_csv.py` — расширенный pipeline (6 сущностей)
- AD ЕПВО: `orders_importer.py` — pipeline с TRANSCRIPT (7 сущностей)
...
> **Расхождение с RF_TFW-5 §3.7:** RF_TFW-5 описывает 4 сущности (ORDERS → ORDER_SECTIONS → SECTION_PERSON → SCHOLARSHIP), но практическая реализация AD ЕПВО добавляет ORDER_STUDENT_INFO и ORDERS_ADDITIONAL. Полный pipeline = 6 обязательных + 1 опциональный (TRANSCRIPT).
...
1. **Детерминистический** — если ID вычислялись по формуле при создании (например, в AD ЕПВО: `order_id = base + studentId + month_offset`)
...
3. **6+1 сущностей для type=13.** RF_TFW-5 §3.7 описывает 4 сущности, но практическая реализация AD ЕПВО использует 6 (+ опционально TRANSCRIPT). В RF задокументирован полный pipeline из 6+1 сущностей.
...
|---|------|--------|-----|----------|
| 1 | RF_TFW-5 §3.7 | 170-181 | gap | Pipeline type=13 описан как 4 сущности, но практика AD ЕПВО показывает 6+1. RF_TFW-5 требует обновления |
