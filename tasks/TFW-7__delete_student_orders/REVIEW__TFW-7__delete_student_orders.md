...
RF полностью покрывает scope из TS. Composite key map для 12 сущностей, 7 шаблонов удаления, детализированный pipeline для type=13 с учётом практики ИС ОВПО. Все открытые вопросы из HL §2.2 помечены ⚠️ с рекомендациями по fallback. Observations ценные — RF_TFW-5 §3.7 действительно нуждается в обновлении.
