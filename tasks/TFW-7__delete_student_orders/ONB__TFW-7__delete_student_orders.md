...
Создать RF-документ, описывающий обратную процедуру (удаление) для приказов ЕПВО. Для каждого типа приказа — какие сущности удалять, в каком порядке, с каким composite key, и какие побочные эффекты откатить. Отдельный детализированный pipeline для type=13 (стипендии) на основе практической реализации ИС ОВПО. Документация нейтральная, для любой ИС ОВПО.
...
| Практическая реализация ИС ОВПО | Практический pipeline стипендии (6 сущностей + month_offset) |
| Практическая реализация ИС ОВПО | Pipeline type=13 с TRANSCRIPT |
...
2. **TRANSCRIPT при type=13**: RF_TFW-5 §3.7 не включает TRANSCRIPT в pipeline стипендии, но некоторые реализации ИС ОВПО создают его с `not_included_scholarship=True`. Если TRANSCRIPT был создан — его тоже нужно удалить. Но если pipeline не создавал TRANSCRIPT, то удалять нечего.
...
1. **RF_TFW-5 §3.7 vs практика ИС ОВПО**: RF_TFW-5 описывает pipeline стипендии как 4 сущности (ORDERS → ORDER_SECTIONS → SECTION_PERSON → SCHOLARSHIP), но практическая реализация ИС ОВПО использует 6: добавляются `ORDER_STUDENT_INFO` и `ORDERS_ADDITIONAL`. В RF задокументирую полный pipeline из 6 сущностей (+ опционально TRANSCRIPT).
